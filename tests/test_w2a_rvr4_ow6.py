...
# --- IG-17 repair: withdrawn-source metadata (Owner-authorized bounded
# --- allowlist extension: engine/deliverable_assembler.py) --------------------

def _withdrawn_meta(state):
    return assemble_deliverable(state)["_session_meta"]["withdrawn_source_records"]


def _decision_only_state():
    """W2-A refinement + withdrawal, but NO withdrawn legacy answer."""
    s = IdeaState(idea_id="pid-1")
    s.gaps.append(Gap(gap_type=PHYSICAL_FEASIBILITY, status=OPEN, opened_at=1))
    s.record_interaction(action=DISPOSITION_ANSWERED, content=ANSWER,
                         gap_context=PHYSICAL_FEASIBILITY)     # active, NOT superseded
    ctx = declare_decision_context(s, "Which latch mechanism should hold it?")
    a1 = declare_alternative(s, "Toggle latch", ctx.record_id)
    a2 = declare_alternative(s, "Spring pin", ctx.record_id)
    # refinement supersedes a1's chain head; withdrawal supersedes a2's
    s.record_interaction(action=a1.disposition, content="Improved toggle latch",
                         decision_context_root=ctx.record_id,
                         supersedes=[a1.record_id])
    withdraw_alternative(s, a2.record_id, "too fiddly")
    return s


def test_ig17a_decision_only_history_produces_no_withdrawn_answer_note():
    meta = _withdrawn_meta(_decision_only_state())
    assert meta["total"] == 0
    assert meta["note"] is None


def test_ig17b_genuine_legacy_correction_metadata_unchanged():
    s = IdeaState(idea_id="pid-1")
    s.gaps.append(Gap(gap_type=PHYSICAL_FEASIBILITY, status=OPEN, opened_at=1))
    first = s.record_interaction(action=DISPOSITION_ANSWERED, content="old answer",
                                 gap_context=PHYSICAL_FEASIBILITY)
    s.record_interaction(action=DISPOSITION_ANSWERED, content="corrected answer",
                         gap_context=PHYSICAL_FEASIBILITY,
                         supersedes=[first.record_id])
    meta = _withdrawn_meta(s)
    assert meta["total"] == 1
    assert meta["note"]                        # real withdrawn answer still surfaced


def test_ig17c_mixed_history_counts_legacy_answers_only():
    s = _decision_only_state()                 # 2 superseded DECISION records
    first = s.record_interaction(action=DISPOSITION_ANSWERED, content="old answer",
                                 gap_context=PHYSICAL_FEASIBILITY)
    s.record_interaction(action=DISPOSITION_ANSWERED, content="corrected answer",
                         gap_context=PHYSICAL_FEASIBILITY,
                         supersedes=[first.record_id])
    superseded = [r for r in s.assertions if r.superseded_by is not None]
    assert len(superseded) == 3                # 2 decision + 1 legacy, by construction
    meta = _withdrawn_meta(s)
    assert meta["total"] == 1                  # ONLY the genuine legacy answer
    assert meta["note"]
