...
the post-W2-B-contract synchronization (base
`48017ec0259e5fc7bcb105e0b018f6d447057bda`, PR #573 — the authoritative
W2-B/RVR-6a implementation-contract merge). Rows touched at a
...
| RVR-6a routing/register core (W2-B) | progression_loop seams per Wave-2 contract §H; implementation contract = `W2_B_RVR6A_IMPLEMENTATION_CONTRACT_CANDIDATE.md` (authoritative via PR #573) | Wave-2 contract §H; OD-R5; W2-B contract (PR #573, `48017ec0…`) | OPEN — **CONTRACT AUTHORITATIVE** (PR #573, accepted repaired candidate `5e91fd9c…`; rejected sibling `0448e36…` preserved as evidence); **IMPLEMENTATION NOT STARTED; IMPLEMENTATION-START NOT AUTHORIZED** (a separate explicit Owner instruction is required — the contract merge is not it) | W2-B implementation-start authorization (separate Owner instruction), then the exact-SHA implementation lifecycle. **W/M timing (corrected citation):** the numeric W/M values are proposed inside the future implementation candidate/evidence pack and frozen at Owner exact-SHA acceptance of THAT candidate — operative source **Wave-2 contract §P** (the earlier pre-gate was withdrawn as circular; §H defines the slice, not the W/M timing); no numeric values are frozen today | before serious release | FRB | W2-B merged with register/suppression/ordering tests green |
