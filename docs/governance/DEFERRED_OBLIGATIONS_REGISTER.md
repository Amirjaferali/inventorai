...
the post-W2-C-contract synchronization (base
`d796b0cd385d8ad2071088d58a89612715aad888`, PR #579 — the authoritative
W2-C/RVR-6b contract merge; prior maintenance gates: the RVR-6a
...
| Item | Source owner | Origin | Disposition | Return trigger | Latest safe gate | Blocking | Closure evidence required |
|---|---|---|---|---|---|---|---|
| W1-N3 — residual relevance false-negative (bounded question-id-scoped attempt; evidenced fallback = safe false-negative) | RVR-6b / W2-C (gap_relevance + WS10 intent content); contract = `W2_C_RVR6B_IMPLEMENTATION_CONTRACT_CANDIDATE.md` §E (PR #579) | Wave-1 closure §4; Wave-2 contract §J | OPEN — **RETURNED AT THE W2-C CONTRACT GATE AND CONSUMED INTO THE AUTHORITATIVE W2-C CONTRACT** (PR #579 §E, adjudicated disposition A — not assumed from the return gate). Bound strategy: `ATTEMPT BOUNDED CLOSURE` OR `EVIDENCED FALLBACK = DEFER AS SAFE FALSE-NEGATIVE`. **NOT satisfied** — the future implementation outcome does not exist yet. Deferral durability bound by the contract: `DEFERRED ≠ SATISFIED` and `DEFERRED ≠ CLOSED BY SUCCESS`; canonical relevance owner remains `gap_relevance` / the RVR-2 family (no duplicate owner); **RVR-7 is the mandatory downstream return/input** (OD-R4 lineage); this row persists through any deferral; release-gate re-examination and FCORA zero-orphan coverage apply | W2-C implementation (the bounded attempt's outcome: closure evidence or the evidenced deferral record with the contract's §E durable-ownership content) | W2-C | CONDITIONAL | W2-C RED test vs frozen S2 R6 fixture passing without new false positives, or evidenced deferral record |
...
| RVR-6b WS10 content + intent-aware completion (W2-C) | WS10 (existing loader) + routing consumers; contract = `W2_C_RVR6B_IMPLEMENTATION_CONTRACT_CANDIDATE.md` (PR #579) | Wave-2 contract §I/§N | OPEN — **CONTRACT AUTHORITATIVE (PR #579, accepted candidate `455cb502…`; Lead-rejected sibling `706917cb…` preserved as evidence); IMPLEMENTATION NOT AUTHORIZED.** **OD-W2-WS10-SCOPE EXERCISED** at the Owner's exact-SHA contract acceptance (ODR Wave-2 §A row): **two per-domain WS10 registry instances** (electronics = existing 11 committed ids; mechanical = existing 10; total 21) through the **unmodified D11/D19 loader**; OD-PDVG-04(a) bounded to exactly those 21 ids; combined-source reconciliation REJECTED; no new identities. Future implementation obligations bound by the contract (owned there, cross-referenced here, not duplicated): combined W2-B×W2-C precedence proposed at the implementation candidate under the full Amendment-1 §6 evidence discipline; lapsed-acceptance stale-index revalidation REQUIRED (outcome `AFFECTED` with evidence or `NOT AFFECTED` with mechanical proof — never omitted by assertion); explicit EN/AR pack evidence; 467/484 family looseness stays unreconciled (the full suite is the authoritative reproduction); W1-N3 bounded attempt (its own row above) | separate explicit Owner W2-C implementation-start authorization (the W2-A/W2-B precedent) | before serious release | FRB | W2-C merged with registry CONTENT covering all 21 existing committed ids — physically represented as two per-domain registry instances — and loader-validated (the historic "21-id registry" phrase here meant content coverage; it never required one combined physical registry) |
