"""Workstream 2 Safety Signal Stabilization — regeneration / comparison harness.

File: docs/governance/evidence/workstream2_safety_stabilization/regenerate_and_compare.py
Provenance: a COPY of the immutable Workstream 1 evidence script
docs/governance/evidence/workstream1_deliverable_baseline/reproduce_baseline.py
(contract §8 / F4: the WS1 script is committed evidence and is never
modified; each workstream copies the harness into its own self-contained
evidence directory and writes ONLY there). All journey inputs below are
byte-identical to the WS1 inputs so the base-versus-head comparison is
like-for-like.
Purpose: regenerate the same two evidence bundles at the Workstream 2
implementation head, where the previously false-negative journey MUST now
produce inventor-stated safety signals.
F3 (contract §7): if the scripted journey does not reach the completion
branch within the iteration bound, this harness prints an explicit
"JOURNEY INCOMPLETE" message, exits non-zero, and writes NO artifact
(all artifact writes happen only after the completion assertion passes).
...
code 2 with "JOURNEY INCOMPLETE" if completion is not reached.
Prohibited behaviors: MUST NOT modify any file outside this directory (never
the Workstream 1 baseline directory); MUST NOT mutate repository state,
scoring, fixtures, or tests; MUST NOT edit, clean up, summarize, or reword
any inventor statement or any engine output (the ONLY transformation applied
is the disclosed normalization of run-random identifiers below).
...
def run_false_negative():
    ...
    # F3 (contract §7): fail loudly if the completion branch was not reached
    # within the iteration bound; write NO artifact from an incomplete journey.
    completed_journey = bool(log) and log[-1].get("page_state") == "COMPLETION_BRANCH"
    if not completed_journey:
        SESSION_STORE.pop(sid, None)
        print("JOURNEY INCOMPLETE — completion branch not reached within "
              "%d iterations; no artifact written (contract §7 / F3)."
              % MAX_ITERATIONS)
        sys.exit(2)
