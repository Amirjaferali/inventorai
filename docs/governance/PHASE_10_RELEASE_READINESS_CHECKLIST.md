...
| RL-A6 | Bounded input hardening | IMPLEMENTED LOCAL FOUNDATION | P10-SEC2; `web/app.py` `MAX_FREE_TEXT_CHARS`, `MAX_CONTENT_LENGTH` | 128 KiB transport bound + 20,000-char cap + NUL rejection on the TWO primary free-text surfaces; residual gaps remain: legacy ILT-002 start routes and criticality/decision-workspace free text are transport-bounded ONLY; NOT "all inputs fully hardened" | production abuse/input review: PSRR-TIME (item 1) |
