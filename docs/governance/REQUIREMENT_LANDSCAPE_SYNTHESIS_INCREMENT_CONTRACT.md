...
- any change to Sections 4, 5, 6, 8, 12, or 14, the safety-signal
...
| D4 Protected-test amendment | Approved with strict limits | The named Increment-4 and Phase-7C tests may be amended only where strictly necessary to add and pin the new owner-approved disposition vocabulary and presentation behavior. No existing protected assertion may be removed, weakened, broadened, replaced, bypassed, or converted into a non-asserting test. Every protected-test amendment must be listed individually in this contract's implementation record and the PR description. |
...
| File | Authorization |
|---|---|
| `engine/requirement_landscape.py` | CHANGE (bounded): extend `_record_kind`, `_ANCHOR_LABELS`, `_SOURCE_STATUS`, and `_ACTION` for the three authorized dispositions; replace the empty-content placeholder wording (D7). Requirement-ID scheme, one-row-per-record, ordering, precedence, and all other vocabulary byte-unchanged. |
...
- **GREEN:** the new focused suite passes with zero skips and zero xfails (a
  ...
  Phase-7A 9, Phase-7B 9, deliverable hygiene 22, structured criticality 18,
