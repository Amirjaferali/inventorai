...
adaptive questioning, not runtime translation, not RVR-8. Current truth:
`RVR-7 CONTRACT OWNER-ACCEPTED: NO`; `RVR-7 IMPLEMENTATION AUTHORIZED: NO`;
`RVR-7 IMPLEMENTATION START AUTHORIZED: NO` (separate explicit Owner instruction required after
contract acceptance); the OD-W2-D-P6-18 display-rule supersession stays `OWNER DECISION REQUIRED`;
