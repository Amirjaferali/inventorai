...
| D-P6-18 | Global UI language selector | ACCEPTED as FUTURE REQUIREMENT — **NOT AUTHORIZED / NOT STARTED** | NONE | A persistent shared-header/navigation global language control applying the selected UI language consistently across all pages without returning to the first page. Implementation is a FUTURE independently-authorized gate — NOT P6-1, NOT this sync — and must not be silently assigned to any Phase-6 increment |
